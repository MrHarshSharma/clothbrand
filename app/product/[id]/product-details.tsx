            ...
            <div className="bg-[#F8F8F8] border-b border-[#EBEBEB]">
                <div className="container mx-auto px-6 py-4">
                    <div className="flex items-center gap-2 text-sm">
                        <Link href="/products" className="text-[#717171] hover:text-[#B8975A] transition-colors">
                            Shop
                        </Link>
                ...
                <div className="grid lg:grid-cols-2 gap-8 lg:gap-16">
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            ...
                            {product.images && product.images.length > 1 && (
                                <>
                                    <button
                                        ...
                                        className="absolute left-3 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-white shadow-md text-[#4A4A4A] hover:text-[#B8975A] transition-all opacity-0 group-hover:opacity-100"
                                    >
                                    ...
                                    <button
                                        ...
                                        onMouseEnter={() => setIsZooming(false)}
                                        onMouseLeave={() => setIsZooming(true)}
                                        className="absolute right-3 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-white shadow-md text-[#4A4A4A] hover:text-[#B8975A] transition-all opacity-0 group-hover:opacity-100"
                                    >
                            ...
                            {/* Badges */}
                            {product.isNew && (
                                <span className="absolute top-4 left-4 px-3 py-1.5 bg-[#1A1A1A] text-white text-xs font-semibold rounded-md z-30">
                                    NEW
                                </span>
                            )}
                    ...
                    <div className="lg:py-4">
                        ...
                        {product.categories && product.categories.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-4">
                                {product.categories.map((category, idx) => (
                                    <span
                                        key={idx}
                                        className="text-xs font-medium text-[#B8975A] uppercase tracking-wide"
                                    >
                                        {category}
                                    </span>
                                ))}
                                    ...
                                    {product.variations.map((variation) => (
                                        <button
                                            ...
                                            className={`px-4 py-3 rounded-lg border transition-all text-sm ${selectedVariation?.id === variation.id
                                                ? 'border-[#B8975A] bg-[#F5F0EB] text-[#B8975A]'
                                                : 'border-[#EBEBEB] bg-white text-[#4A4A4A] hover:border-[#B8975A]/50'
                                                }`}
                                        >
                        ...
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div className="flex items-center border border-[#EBEBEB] rounded-lg flex-shrink-0">
                                <button
                                    ...
                                    className="p-3 hover:bg-[#F8F8F8] transition-colors text-[#4A4A4A] hover:text-[#B8975A]"
                                    disabled={quantity <= 1}
                                >
                        ...
                        <div className="grid grid-cols-2 gap-4 p-4 bg-[#F8F8F8] rounded-xl mb-8">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-[#B8975A]">
